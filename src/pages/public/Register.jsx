...
import { schoolCampus, schoolName } from "../../constants/strings";
...
function Register() {
  ...
  const handleOnChange = (e) => {
    const currentFieldError = authValidation.validateRegistration({
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const formErrors = authValidation.validateRegistration(value);
    if (Object.keys(formErrors).length > 0) {
      ...
      try {
        ...
        setError({ ...error, authError: errorMessage.firebaseError });
        setLoading(false);
      }
    }
  };
